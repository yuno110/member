...
- [x] `application-local.yml`에 `INTERNAL_API_KEY` 추가 (board와 같은 값)
- [x] **공개키** `jwt-public.pem`을 `src/main/resources/`에 배치 — **실제 키다.** 개인키는 두지 않는다

> **M-04는 테스트용 키 페어를 만들 필요가 없다.** 실제 공개키가 이미 배치되어 있고 짝이 되는 개인키는 auth-service가 갖는다. I-01은 여전히 **두 사본이 같은 키인지**와 **auth가 실제로 발급한 토큰을 받는지**를 확인한다.
- [x] 문서 저장소 클론 (`../sp-docs`)
